...
def pdf_to_lines(pdf_file_path):
    ...
    # print(text)
    page_lines = [l.strip() for l in text.split(".") if l.strip()]
    # print(page_lines)
    # length = len(page_lines)
    lines.extend(page_lines)
    # length_of_sentence.extend(length)
    return lines
...
def record_audio(duration, sample_rate=16000):
    st.write("Recording...")
    audio_data = sd.rec(int((duration+3) * sample_rate),
...
def main():
    ...
    if st.button("Read This Line"):
        # Record audio
        audio_path = record_audio(len(lines[st.session_state.current_line_idx].split(" "))//2)
        start = time.time()
        try:
